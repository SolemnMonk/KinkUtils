...
import monk.solemn.k.utils.enums.Gender;
...
public class ActorUtils {
	...
	public static Actor getActorData(WebDriver driver, String fallbackName) throws SQLException {
		...
		if (actorFoundPage) {
			...
			logger.traceExit();
			WicketApplication.getActorDao().saveActor(actor);
			return actor;
		}
		...
		for (WebElement imageElement : imageElements) {
			try {
				...
				if (!imageExists(imageBuffer, actorImages)) {
					image = new KUtilsImage();
					
					image.setFilePath(Paths.get("Actors", actorName, "Images", (actorImages.size() + 1) + ".png").toString());
	...
	private static boolean imageExists(RenderedImage image, List<KUtilsImage> actorImages) {
		...
		try (ByteArrayOutputStream hashOutput = new ByteArrayOutputStream()) {
			...
			byte[] imageBytes = hashOutput.toByteArray();
	...
	private static String hashImage(byte[] imageBytes) {
		String hash = null;
