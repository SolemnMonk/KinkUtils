...
public class KinkPlugin implements PluginBase, NetworkBase, SeleniumPlugin {
	...
	public KinkPlugin(BundleContext context) {
		initialize();
	}
	...
	public void run() {
		if (queuedTask.getTask().getAction() == Action.Rip) {
			...
			Downloader.performDownload(driver, queuedTask);
		} else if (queuedTask.getTask().getAction() == Action.GatherData) {
			DataGatherer.gatherData(driver, queuedTask);
		}
	}
