...
public class LaunchSelenium extends WebPage {
	...
	protected void onInitialize() {
		...
		Link reloadPluginsLink = new Link("reloadPluginsLink") {
			public void onClick() {
				((WicketApplication) WicketApplication.get()).reloadPlugins();
			};
		};
	...
	public void launchKtsSeleniumClick() throws InterruptedException, IOException, ParseException, SQLException {
		...
		WicketApplication wApp = (WicketApplication) WicketApplication.get();
...
//		}
//		
		((WicketApplication) WicketApplication.get()).getPluginManager().getExtensions(SeleniumAuthentication.class, "kink-plugin").get(0).logout(driver);
		exit();
	}
